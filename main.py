...
def main():
    ...
    all_datasources = fetch_all_datasources(base_url, headers)
    for ds in all_datasources:
        ds_id = ds['id']
        print(f'Checking status of MDE for ds_id: {ds_id}')

        # 2. Get status of last MDE job
        did_last_mde_job_succeed = did_last_mde_job_succeed_by_ds_id(ds_id, base_url, headers)
        
        if did_last_mde_job_succeed == True:
            print('succeeded!')
        else:
            # 3. If applicable, email datasource admins
            ds_owner_email_addresses = get_email_owner_addresses_by_ds(ds, base_url, headers)
            email_admins_by_email_address(ds, ds_owner_email_addresses)
